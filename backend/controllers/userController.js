...
import {sendEmail} from "../utils/helpers/sendOtp.js";
...
const searchUserProfile = async (req, res) => {
  ...
  try {
    ...
    user = await User.find({ username: regexQuery })
      .select("-password")
      .select("-updatedAt");
...
const signupUser = async (req, res) => {
  try {
    ...
    // Create new user object (not saved yet)
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px; background-color: #f9f9f9;">
        <h2 style="color: #2c3e50;">Welcome to <span style="color: #1abc9c;">Tech Thapar</span> 👋</h2>
        <p style="font-size: 16px; color: #333;">Here is your One-Time Password (OTP):</p>
        <div style="margin: 20px 0; text-align: center;">
          <span style="display: inline-block; background-color: #1abc9c; color: #fff; padding: 12px 30px; font-size: 24px; border-radius: 8px; letter-spacing: 4px;">
            ${otp}
          </span>
        </div>
        ...
        <p style="font-size: 12px; color: #aaa;">Thanks, <br> The Tech Thapar Team</p>
      </div>
    `;

    try {
      await sendEmail(email, "Your Tech Thapar OTP", html);
    } catch (emailErr) {
    ...
    // Optional: Generate token (can be done after verification instead)
